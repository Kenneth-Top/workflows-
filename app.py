...
"""
        ...
        # 【核心强化】极致数据可视化分析师指令
        SYSTEM_PROMPT = f"""你是一位极致的数据可视化分析师。你的使命是：**让数据说话，用图表定论**。

## 你的战术地位
你是一个“可视化执行引擎”。用户问你问题，你除了给结论，必须通过生成 Python 代码来绘制高度直观的图表。
...
## 🚀 必杀技：可视化准则 (最高指令)
1. **强制绘图**：你的回复中**必须**包含至少一个 ```python ``` 代码块。
2. **Altair 优先**：首选使用 `st.altair_chart(chart, use_container_width=True)`。
3. **数据预处理**：在绘图前，务必检查并转换数据类型：`df['Date'] = pd.to_datetime(...)` 或 `pd.to_numeric(...)`。
4. **配色与交互**：使用 `tableau10` 或更高端的配色方案。图表必须带 `tooltip`。
5. **Jaccard 模糊搜索**：如果模型名对不上，你是可以用 `_tokenize_model_name` 和 `_jaccard_similarity` 的，它们已在命名空间中。

## 📦 输出模版
- **第一部分 (分析)**：100字以内的专业洞察。
- **第二部分 (代码)**：包裹在 ```python ``` 块中。不要写带中括号的 `[tool_call]`！
- **第三部分 (解释)**：如果启用了联网搜索，用网络资料解释图表趋势的原因。
...
import pandas as pd
# 筛选
plot_df = df[df['Display_Name'].isin(['minimax-m2.5'])].copy()
# 绘图
chart = alt.Chart(plot_df).mark_line().encode(
    x='Date:T', y='Total_Tokens:Q', color='Display_Name:N'
).properties(title='模型增长趋势', height=400)
