...
请务必参考以下风格撰写（将其作为你的格式模板）：
“本周受 [某外部事件/节点] 影响，[国内外/某特定分类] 大模型推新 [加快/放缓]，共推出 [X] 款模型。其中，[模型A] 上量速度位居第一，近7日均值达到 [X] B，动量高达 [X]。经查证，这是因为其推出了 [具体的优惠政策/能力更新]；同时，近期新上线的一批模型中，最值得关注的是 [模型C] 和 [模型D]，其核心性能指标 [排名/基准] 达到 [X]，而价格低至 [输入X/输出X]，对现有市场格局带来一定影响。另外值得注意的是，[模型E] 增速明显停滞，推测与 [具体退市/商业化策略] 有关。”

任务规范：
1. 必须使用准确的上方数据（后缀B = 10亿Tokens）。
2. 在陈述原因时，不要凭空猜测，必须通过网络搜索出确切的外部事件（如：某大厂在几号宣布了什么API免费计划、发布了什么新模型版本）。
3. 保持客观的定量视角，坚决剔除诸如“引爆全球调用”、“形成硬件飞轮”等假大空的抒情和主观拔高词汇。
        """
